...
test_that("no generic",{
  ...
  expect_equal(printer(R62S3_NoGeneric$new(), "Hello World"), "Hello World")
  expect_error(excluder(R62S3_NoGeneric$new()))
  expect_equal(Status(R62S3_NoGeneric$new()), "Printing")
})

test_that("S3 generic",{
  expect_silent(R62S3(R62S3_S3Generic, assignEnvir = topenv(), exclude = "excluder", scope = c("public","active")))
  expect_equal(lower(R62S3_S3Generic$new(), "HELLO WORLD"), "hello world")
  expect_error(excluder(R62S3_S3Generic$new()))
  expect_equal(Status(R62S3_S3Generic$new()), "Printing")
})
